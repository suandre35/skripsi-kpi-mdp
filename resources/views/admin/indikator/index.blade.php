                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li aria-current="page">
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-sm font-medium text-gray-800 dark:text-gray-100">Indikator</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 dark:border-gray-700">
                    ...
                    <div class="p-6 border-b border-gray-100 dark:border-gray-700">
                        <div class="flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                            ...
                            <form method="GET" action="{{ route('indikator.index') }}" class="flex flex-col md:flex-row gap-3 w-full xl:w-auto">
                                ...
                                <a href="{{ route('indikator.index') }}" class="flex items-center justify-center px-4 py-2.5 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700">
                                    Reset
                                </a>
                                @endif
                            </form>
                            ...
                            <div class="flex-shrink-0">
                                <a href="{{ route('indikator.create') }}" class="flex items-center justify-center gap-2 text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 transition transform hover:-translate-y-0.5 shadow-lg shadow-blue-500/30 w-full xl:w-auto">
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-4">Target Divisi</th>
                                ...
                                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition duration-150">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-start gap-3">
                                            ...
                                            <div>
                                                <div class="text-sm font-bold text-gray-900 dark:text-white">{{ $indikator->nama_indikator }}</div>
                                                <p class="text-xs text-gray-500 dark:text-gray-400 line-clamp-1 mt-0.5" title="{{ $indikator->deskripsi }}">
                                                    {{ $indikator->deskripsi ?? 'Tidak ada deskripsi' }}
                                                </p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-900 dark:text-purple-300">
                                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M17.707 9.293a1 1 0 010 1.414l-7 7a1 1 0 01-1.414 0l-7-7A.997.997 0 012 10V5a3 3 0 013-3h5c.256 0 .512.098.707.293l7 7zM5 6a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd"></path></svg>
                                            {{ $indikator->kategori->nama_kategori ?? '-' }}
                                        </span>
                                    </td>

                                    {{-- KOLOM 3: TARGET DIVISI --}}
                                    <td class="px-6 py-4">
                                        <div class="flex flex-wrap gap-1.5">
                                            @if(!empty($indikator->target_divisi))
                                                @foreach($indikator->target_divisi as $idDiv)
                                                    {{-- Mapping ID ke Nama Divisi dari $allDivisi --}}
                                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200 dark:bg-blue-900/30 dark:text-blue-300 dark:border-blue-800">
                                                        {{ $allDivisi[$idDiv] ?? 'ID:'.$idDiv }}
                                                    </span>
                                                @endforeach
                                            @else
                                                <span class="text-xs italic text-red-500 flex items-center gap-1">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                        {{ $indikator->satuan_pengukuran ?? '-' }}
                                    </td>

                                    {{-- KOLOM 5: STATUS --}}
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="h-2.5 w-2.5 rounded-full mr-2 {{ $indikator->status ? 'bg-green-500 animate-pulse' : 'bg-red-500' }}"></div>
                                            <span class="text-sm font-medium {{ $indikator->status ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <a href="{{ route('indikator.edit', $indikator->id_indikator) }}" class="p-2 text-indigo-600 hover:text-white hover:bg-indigo-600 rounded-lg transition-colors duration-200" title="Edit Indikator">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </a>

                                            {{-- TOMBOL HAPUS DENGAN MODAL --}}
                                            <button @click="showDeleteModal = true; deleteUrl = '{{ route('indikator.destroy', $indikator->id_indikator) }}'"
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-lg font-medium">Data indikator tidak ditemukan.</p>
                                            <p class="text-sm">Silakan tambah indikator baru.</p>
                                        </div>
                                    </td>
                                </tr>
                                @endforelse
